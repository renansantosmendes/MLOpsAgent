    ...
    class DummyReport:
        ...
        def run(self, *args, **kwargs):
            return DummySnapshot({
                "metrics": [
                    {"metric_name": "ValueDrift", "value": 0.1, "config": {"column": "col1", "threshold": 0.2}}
                ]
            })

    monkeypatch.setattr(drift, "Report", DummyReport)

    result = drift.data_drift_tool(reference_data=reference_data, new_data=new_data, drift_share_threshold=0.2)
